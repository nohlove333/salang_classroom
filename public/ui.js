(function () {
  ...
  async function resolveFileUrl(file, role, allowExternalAccess) {
    ...
    if ((window.LEARN_CONFIG || {}).directUpload) {
      var directAccess = await window.LearnAPI.request('prepareFileAccess', { fileId: file.id, intent: 'preview' }, role);
      return directAccess.previewUrl || directAccess.publicUrl;
    }
  ...
  async function previewAttachment(file, role, context, allowDownload) {
    ...
    try {
      if (isImage) {
        ...
        if (!mayDownload) {
          var restrictedOfficeAccess = await temporaryAccess(file, role, 'preview');
          if (restrictedOfficeAccess.previewUrl) {
            stage.innerHTML = '<iframe src="' + attr(restrictedOfficeAccess.previewUrl) + '" title="' + attr(file.name) + ' 미리보기" allow="autoplay"></iframe>';
          } else {
        ...
        if (officeAccess.previewUnsupported || !officeAccess.previewUrl) {
          stage.innerHTML = mayDownload
            ? '<div class="preview-message"><strong>이 파일은 저장해서 확인해 주세요.</strong>' +
                '<p>Word·PowerPoint·Excel 파일은 브라우저 호환 상태에 따라 저장해서 확인해야 할 수 있어요.</p>' +
  ...
  function thumbnailUrl(file, role) {
    ...
    thumbnailCache[cacheKey] = queueThumbnailRequest(function () {
      if ((window.LEARN_CONFIG || {}).directUpload) {
        return window.LearnAPI.request('prepareFileAccess', { fileId: file.id, intent: 'preview' }, role);
      }
      return window.LearnAPI.request('getFileContent', { fileId: file.id }, role);
    }).then(function (response) {
      if (response.previewUrl || response.publicUrl) return response.previewUrl || response.publicUrl;
