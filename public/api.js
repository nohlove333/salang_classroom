(function () {
  ...
  async function filesToPayload(files) {
    ...
    if (config.directUpload && !config.demoMode) {
      return Promise.all(prepared.map(uploadDirect));
    }
  ...
  async function uploadDirect(file) {
    ...
    try {
      ...
      throw new ApiError('파일 서버에 연결하지 못했습니다.', 'UPLOAD_NETWORK_ERROR');
    }
    var result;
    try { result = await response.json(); } catch (error) { result = null; }
    if (!response.ok || !result || !result.ok) {
      throw new ApiError(result && result.message || '파일을 업로드하지 못했습니다.', result && result.code || ('HTTP_' + response.status));
    }
    return result.data;
  }
  ...
  AppAPI.prototype.request = async function (action, payload, role, retryCount) {
    ...
    if (!config.apiUrl || config.apiUrl.indexOf('PASTE_') === 0) {
      throw new ApiError('Cloudflare API 주소가 아직 설정되지 않았습니다.', 'NOT_CONFIGURED');
    }
